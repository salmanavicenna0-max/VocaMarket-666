...
@section('content')
    ...
    <!-- Carousel Section -->
        ...
        <div class="container mx-auto px-4 relative z-10 pt-8 pb-16 reveal">
            ...
            <!-- Product Grid: 2 baris, tiap baris scroll horizontal sendiri -->
            @php
                $rowTop = [];
                $rowBottom = [];
                foreach ($products as $i => $product) {
                    if ($i % 2 === 0) { $rowTop[] = $product; }
                    else { $rowBottom[] = $product; }
                }
            @endphp
            <div class="relative mb-8 space-y-3">
                <div class="overflow-x-auto rec-wrap pb-2 snap-x snap-mandatory scroll-smooth" style="scrollbar-width: thin;">
                <div class="grid rec-grid gap-2">
                @foreach($rowTop as $product)
